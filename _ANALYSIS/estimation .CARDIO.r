...
# Overall Conclusion RE Cardio: nothing going on here


ANSAMP.CARDIO.occ <- read_csv('_DATA/ANSAMP.occ.csv') %>%
  filter(spec_prim_1_1=='06') %>%
  mutate(
         first.treat.rsa = ifelse(first.treat==0, Inf, first.treat)
         )
ANSAMP.CARDIO.rep <- read_csv('_DATA/ANSAMP.rep.csv') %>%
  filter(spec_prim_1_1=='06') %>%
  mutate(
         first.treat.rsa = ifelse(first.treat==0, Inf, first.treat)
         )
ANSAMP.CARDIO.sf <- read_csv('_DATA/ANSAMP.sf.csv') %>%
  filter(spec_prim_1_1=='06') %>%
  mutate(
         first.treat.rsa = ifelse(first.treat==0, Inf, first.treat)
         )
ANSAMP.CARDIO.fdr <- read_csv('_DATA/ANSAMP.fdr.csv') %>%
  filter(spec_prim_1_1=='06') %>%
  mutate(
         first.treat.rsa = ifelse(first.treat==0, Inf, first.treat)
         )
...
ggplot() +
  geom_histogram(data=filter(ANSAMP.CARDIO.rep, treated==0),
                 aes(x=pat.vol_l2y_1_pre.samp, y = stat(count / sum(count))),
                 color='blue') +
  geom_histogram(data=filter(ANSAMP.CARDIO.rep, treated==1),
                 aes(x=pat.vol_l2y_1_pre.samp, y = stat(count / sum(count))),
                 color='red')
...
for (i in timing){
  for (j in outcomes){
    ...
    att.i.j.uncond <- att_gt(yname=j,
                             ...
                             gname='first.treat',
                             xformla=NULL,
                             data=get(paste0('ANSAMP.CARDIO.', i))
                             )
...
results.att.gt.CARDIO <- mget(ls(pattern='att.gt.'))

for (i in 1:length(results.att.gt.CARDIO)) {
  
  a <- aggte(results.att.gt.CARDIO[[i]], type='group')
  b <- a$overall.att
  c <- a$overall.se
  ...
  d <- data.frame(estimate=b, se=c) %>%
    mutate(lower=estimate-1.96*se,
           ...
           model=names(results.att.gt.CARDIO)[[i]]
           )
  
  assign(paste0('results.overall.att.gt.', names(results.att.gt.CARDIO)[[i]]), d)
  
  a.p <- aggte(results.att.gt.CARDIO[[i]], type='dynamic') 
  ...
  h <- data.frame(estimate=e, se=f) %>%
    mutate(lower=estimate-g*se,
           ...
           assign(paste0('results.estud.cs.', names(results.att.gt.CARDIO)[[i]]), h)

}


results.cs.overall.CARDIO.tab <- rbindlist(mget(ls(pattern='results.overall.att.gt.')))

results.cs.estud.CARDIO.tab <- rbindlist(mget(ls(pattern='results.estud.cs.')))
...
cs.estud.overview.CARDIO <- results.cs.estud.CARDIO.tab %>%
  group_by(model) %>%
...
for (i in timing){
  for (j in outcomes){
    ...
    r_sa.2023.i.j.overall.all <- staggered(get(paste0('ANSAMP.CARDIO.', i)),
                                      i='npi1',
                                      t='year',
                                      g='first.treat.rsa',
                                      y=j,
    ...
    r_sa.2023.i.j.overall.trtonly <- staggered(filter(get(paste0('ANSAMP.CARDIO.', i)), 
...
results.rsa.overall.CARDIO.tab <- rbindlist(mget(ls(pattern = 'r_sa.2023.')))

rm(list=ls(pattern='r_sa.2023.(occ|rep|sf|fdr).(hhi|lhhi|n_sources|vol_shared).overall.'))


# sig.rsa.overall.CARDIO <- results.rsa.overall.CARDIO.tab %>%
#   filter(significant==1)

for (i in timing){
  for (j in outcomes){
    
    r_sa.2023.i.j.estud.all <- staggered(get(paste0('ANSAMP.CARDIO.', i)),
                                         i='npi1',
                                         t='year',
                                         g='first.treat.rsa',
                                         y=j,
    ...
    r_sa.2023.i.j.estud.trtonly <- staggered(filter(get(paste0('ANSAMP.CARDIO.', i)), 
                                            is.infinite(first.treat.rsa)==F),
                                     i='npi1',
                                     t='year',
                                     g='first.treat.rsa',
...
results.rsa.estud.CARDIO.tab <- rbindlist(mget(results.to.get))

rsa.estud.CARDIO.overview <- results.rsa.estud.CARDIO.tab %>%
  group_by(model) %>%
  summarize(n_significant=sum(significant),
